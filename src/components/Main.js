...
export default function Main(props) {
    ...
    const {cards, onEditProfile, onAddPlace, onEditAvatar, onCardDelete, onCardLike, onCard} = props;
    ...
    return(
        <main className="content page__content">
            ...
            <section className="elements" aria-label="Фото">
                {cards.map((card) => (
                    <div key={card._id}>
                        <Card card={card} onCardClick={onCard} onDeleteClick={onCardDelete} onLikeClick={onCardLike}/>
                    </div>
                ))}
            </section>
        </main>
