    ...
    return (
        <div className={`popup ${isOpen ? "popup_opened" : ""}`}>
            ...
            <button className='popup__close-button' type="button" aria-label="Закрыть окно" onClick={onClose}></button>
                <img src={ isSuccessfully ? success : error } alt='Статус регистрации'></img>
                <h3 className="popup__title popup__title_type_modal">{ isSuccessfully ? message.happy : message.sad }</h3>
            </div>
        </div>
    )
}