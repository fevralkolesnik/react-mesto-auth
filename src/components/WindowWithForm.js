...
export default function WindowWithForm (props) {
    ...
    return (
        <div className={`form form__${name}`}>
            ...
            <form className={`form__form popup__form_type_${name}`} name={name} onSubmit={handleSubmit}>
                ...
                <button className="form__submit-button" type="submit" aria-label="Отправить форму">{buttonText}</button>
            </form>
            {children}
        </div>
    ) 
